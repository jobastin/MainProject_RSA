        ...
        <div class="row">
              ...
              <table class="table table-striped table-hover">
                    ...
                    <th>Action</th>
        				...
        				<tbody id="myTable">
              <?php
                    $qs2=mysqli_query($con,"SELECT * FROM `tbl_user`")or die("Sign in Error");
                    ...
                    ?>
        					  <tr>
        						<td><?php echo $sv2['name']; ?></td>
        						<td><?php echo $sv2['email']; ?></td>
        						<td><?php echo $sv2['phone']; ?></td>
        						<td><?php echo $sv2['gender']; ?></td>
                    <td><?php echo $sv2['dob']; ?></td>
        						<td>
                      <a href="#" id="<?php echo $sv2['login_id']; ?>" class="CustViewMore" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="View More">remove_red_eye</i></a>
                      ...
                      {
                      ?>
                      <a href="#" id="<?php echo $sv2['login_id']; ?>" class="delete deleteCust" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
                      <?php
                      }
                      else {
                        ?>
                        <a href="#reModel" id="<?php echo $sv2['login_id']; ?>" class="reassig reassignCust" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Make as Active">autorenew</i></a>
                        <?php
                      }
                      ?>
                    </td>
...
<!-- Mech view on Map Modal  -->
<div id="CustSendMailModel" class="modal fade">
<div class="modal-dialog">
		<div class="modal-content">
			<form method="post" id="form8">
				...
				<div class="modal-body" id="CustSendMailBody">
          ...
          <h3>To : </h3>
          <fieldset>
          <input class="form-control" value="" disabled="true" type="text" name="email4" id="email4" tabindex="2" required>
          </fieldset>
          <br>
          <br>
          <fieldset>
...
$(document).ready(function(){
   var h_id;
   $('.CustViewMore').click(function() {
      var h_id = $(this).attr("id");
       $.ajax({
               url:"CustViewMore.php",
               method:"post",
               data:{h_id:h_id},
               success:function(data){
                    $('#CustViewMoreBody').html(data);
                    $('#CustViewMoreModel').modal("show");
               }
          });
   });
});
...
$(document).ready(function(){
  var e_id;
   $('.CustSendMail').click(function() {
      e_id = $(this).attr("id");
       $.ajax({
               url:"CustSendMail.php",
               method:"post",
               data:{e_id:e_id},
               success:function(data){
                    $("#email4").val(data);
                    $('#CustSendMailModel').modal("show");
               }
          });
   });
