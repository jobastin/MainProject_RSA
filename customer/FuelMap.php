...
else
{
  ...
  $result=mysqli_query($con,"SELECT * FROM `tbl_fuel` WHERE `Fstatus`=1")or die("Sign in Error");
   ...
   <?php if(mysqli_num_rows($result) > 0){
       while($row = $result->fetch_assoc()){
         ...
         $cn=$c2['Name'];
           ...
           echo '["'.$row['Fcompany'].'", '.$row['Flat'].', '.$row['Flon'].', "'.$row['Femail'].'","'.$row['Fphno'].'",
           "'.$row['Flicense'].'","'.$row['Fadd'].'","'.$row['Fpin'].'","'.$sn.'","'.$cn.'","'.$row['Fid'].'","'.$row['login_id'].'",],';
       }
   }
   ?>
 ];
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <input type="submit" class="btn btn-primary Fsubmit1" id="Fsubmit1" value="Update" />
      </div>
    </div>
  </div>
</div>
...
$(document).ready(function(){
  var p_id;
   ...
   $('.Fsubmit1').click(function() {
     var Pplace = $('#Frequest').val();
     var Mstate = $('#Fdesc').val();
     if(Pplace=="")
     {
       $("#Frequest").css("border-color","red");
     }
     if(Mstate=="")
     {
     ...
     if(!(Pplace==""))
     {
       $("#Frequest").css("border-color","#dddddd");
     }
     if(!(Mstate==""))
     {
       $("#Fdesc").css("border-color","#dddddd");
     }
     if(!(Pplace=="") && !(Mstate=="") )
     {
       $("#Frequest").css("border-color","#dddddd");
       ...
       $.ajax({
               url:"FuelSRegiester.php",
               method:"post",
               data:{rid:rid,login:login,myLat:myLat,myLong:myLong,Pplace:Pplace,Mstate:Mstate},
               success:function(data){
                 window.location.href = 'FuelRequest.php';
               }
          });
     }
    });
