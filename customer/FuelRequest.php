       ...
       <div class="container-fluid p-0 px-lg-0 px-md-0">
		...
		<div class="table-wrapper">
            ...
            <th>Reason</th>
				...
				<tbody>
          <?php
          $qs2=mysqli_query($con,"SELECT * FROM `tbl_fuel_service_request` WHERE `user_id`=$lid  ORDER BY `req_id` DESC ")or die("Sign in Error");
          ...
          ?>
					<tr>
						<td><?php echo "R".$sv2['req_id']; ?></td>
						<td><?php echo $sv2['description']; ?></td>
            <?php
            $sno=$sv2['Sno'];
              $qs3=mysqli_query($con,"SELECT * FROM `tbl_services` WHERE `Sno`=$sno")or die("Sign in Error");
              $sv3 = mysqli_fetch_array($qs3);
            ?>
						<td><?php echo $sv3['Sname']; ?></td>
            <?php
            $fno=$sv2['Fid'];
              $qs4=mysqli_query($con,"  SELECT * FROM `tbl_fuel` WHERE `Fid`=$fno")or die("Sign in Error");
              $sv4= mysqli_fetch_array($qs4);
            ?>
						<td><?php echo $sv4['Fcompany']; ?></td>
            <td><?php echo $sv4['Fphno']; ?></td>
            <?php
            if($sv2['req_status'])
            {
            ?>
            <td style="background-color:#ffc2b3;"><center>Service Pending</center></td>
            <?php
            }
            else{
              ?>
              <td style="background-color:#b3ffb3;"><center>Completed</center></td>
              <?php
            }
            ?>
            <td><?php echo $sv2['comment']; ?></td>
						<td>
              <?php
              if($sv2['status'])
              {
              ?>
              <a href="#" id="<?php echo $sv2['req_id']; ?>" class="delete deleteCust" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
              <!-- <a href="#" id="<?php echo $sv2['Femail']; ?>" class="CustSendMail" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Send a Mail">mail</i></a> -->
              <?php
            }
            else {
              ?>
        ...
        ?>
        <tr>
        <td colspan="6"><center>Not Yet Added  !!!!</center></td>
        </tr>
        <?php
      }
      ?>
