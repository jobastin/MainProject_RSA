...
else
{
  ...
  $result=mysqli_query($con,"SELECT * FROM `tbl_police` WHERE `Pstatus`=1")or die("Sign in Error");
  ?>
<html lang="en">
  <head>
   ...
   <?php if(mysqli_num_rows($result) > 0){
       while($row = $result->fetch_assoc()){
         ...
         $cn=$c2['Name'];
           ...
           echo '["'.$row['Pplace'].'", '.$row['Plat'].', '.$row['Plon'].', "'.$row['Pphno'].'","'.$row['Padd'].'","'.$sn.'","'.$cn.'","'.$row['Ppin'].'",],';
       }
   }
   ?>
 ];
       ...
       google.maps.event.addListener(marker, 'click', (function(marker, i) {
           return function() {
               infoWindow.setContent('<div id="iw-container">' +
                                '<div class="iw-title">'+markers[i][0]+'<input type="button"  data-toggle="modal" data-target="#exampleModal" style="float:right;" value="Report Accident" class="btn btn-success makeRequest" onclick="myFunction('+markers[i][10]+','+markers[i][11]+')" /></div>' +
                                '<div class="iw-content">' +
                                  '<div class="iw-subTitle"><b>Contact Number :</b>'+markers[i][3]+'</div><br>' +
                                  '<div class="iw-subTitle"><b>Address</b></div>' +
                                  '<br><p>'+markers[i][4]+'<br>State :'+markers[i][5]+'<br>'+
                                  'District :'+markers[i][6]+'<br>Pin :'+markers[i][7]+'</p>'+
                                '</div>');
